declare const tableAction: HTMLTableElement;
...
actionCategories.forEach((categoryName, categoryIndex) => {
 ...
 for (let actionIndex = 0; actionIndex < maxRows; ++actionIndex) {
  ...
  for (
   ...
   difficultyIndex < tableActionLevel.length;
   ++difficultyIndex
  ) {
   var action = tr.insertCell();
   action.textContent =
    tableActionLevel[difficultyIndex][categoryIndex][actionIndex];
  }
 }

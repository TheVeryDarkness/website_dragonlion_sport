...
function displayAs (node, mode) {
  ...
  if (mode == 'html5') {
    ...
    { embededFrame.src = node.frame + '&high_quality=1&t=' + videoTimeBegin } else if (node.from == 'vqq')
    // See https://m.v.qq.com/txp/v3/src/iframeapi/new.html
    { embededFrame.src = node.frame + '&show1080p=1&starttime=' + videoTimeBegin } else if (node.from == 'youku')
    ...
    { embededFrame.src = node.frame + '?starttime=' + videoTimeBegin + '&endtime=' + videoTimeEnd } else {
...
function loadFromSelected () {
  ...
  for (let index = 0; index < nodes.length; index++) {
    ...
    if (selected !== 0 && node[selected]) {
      while (info.childNodes.length > 0) { info.removeChild(info.childNodes[0]) }
      {

  ...
  switch (key) {
    ...
    case 'range':
      ...
      if (!(element instanceof Array)) {
        ...
        if (element.length > 2) { node[key] = element.slice(0, 2) } else if (element.length == 1) { node[key] = [element[0], element[0]] } else { node[key] = [0, 0] }
      }
      element = node[key]
    ...
    case 'comment':
      ...
      return form
    default:
      if (typeof (element) !== 'string') { console.error("It's recommended to provide another tool for non-string preperty '", key, "'.") }
